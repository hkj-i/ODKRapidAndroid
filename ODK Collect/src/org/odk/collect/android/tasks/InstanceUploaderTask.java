...
public class InstanceUploaderTask extends AsyncTask<Long, Integer, InstanceUploaderTask.Outcome> {
    ...
    private boolean uploadOneSubmission(String urlString, String id, String instanceFilePath,
        ...
        // hee
        Log.i("InstanceUploaderTask: ", "finished uploading instance");
        
        
        // change the sent thing
        // odk
        String instance_uri = (InstanceColumns.CONTENT_URI).toString() + "/"+ id;
        String[] stringArgs = {id};
        Log.i("InstanceUploaderTask: ", "uri string is = " + instance_uri);
       // look for the message - which message does this belong to?
		Cursor messageRow = Collect.getInstance().getApplicationContext().getContentResolver().query(Uri.parse("content://org.rapidandroid.provider.RapidSms/message"), 
				null, 
				"form_uri = ?", 
				stringArgs, 
				null); 
		...
		if (messageRow != null) {
			...
			rapidValues.put("is_sent", 1);
			int colsChanged = rapidResolver.update(Uri.parse("content://org.rapidandroid.provider.RapidSms/message"),
		    		 rapidValues,
		    		 "form_uri = " + id,
		    		 null);
			Log.i("InstanceUploaderTask: ", "changed issent");
		} else {
			// the instance file doesn't exist in rapidandroid db
			// TODO
		}
        return true;
    }
