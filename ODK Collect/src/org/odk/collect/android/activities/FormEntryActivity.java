...
public class FormEntryActivity extends Activity implements AnimationListener,
		...
		if (data instanceof FormLoaderTask) {
			...
			if (intent != null) {
				...
				if (getContentResolver().getType(uri) == InstanceColumns.CONTENT_ITEM_TYPE) {
					...
					// hee get the form_name too
					//
					Log.i("FOrmEntryActivity", "getting formid and version for this instance...");
				//	String jrFormId = null;
						...
						try {
							...
							if (instanceCursor.getCount() != 1) {
								...
								/*jrFormId = instanceCursor
										.getString(instanceCursor
												.getColumnIndex(InstanceColumns.JR_FORM_ID));*/ // hee
								jrFormName = instanceCursor
										.getString(instanceCursor
												.getColumnIndex(InstanceColumns.JR_FORM_NAME));
					...
					if (jrVersion == null) {
						...
						selectionArgs = new String[] { jrFormName};
						//selection = FormsColumns.JR_FORM_ID + "=? AND "
						selection = FormsColumns.DISPLAY_NAME + "=? AND "
								+ FormsColumns.JR_VERSION + " IS NULL";
						// hee hopefully add jr form name too
					} else {
						// hee
						//selectionArgs = new String[] { jrFormId, jrVersion };
						selectionArgs = new String[] { jrFormName, jrVersion };
						//selection = FormsColumns.JR_FORM_ID + "=? AND "
						selection = FormsColumns.DISPLAY_NAME + "=? AND "
	...
	private boolean saveDataToDisk(boolean exit, boolean complete,
		...
		Log.i("FormEntryActivity", "Trying to save to disk");
		...
		Log.i("FormEntryActivity", "after executing");
		return true;
	}
